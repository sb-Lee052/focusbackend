...
@api_view(['GET'])
@permission_classes([IsAuthenticated])
def last_session_view(request):
    sess = StudySession.objects.filter(user=request.user, end_at__isnull=False).order_by('-end_at').first()
    if not sess:
        return Response({'session_id': None})
    return Response({'session_id': sess.id})